...
namespace NBB.Messaging.Nats
{
    public class JetstreamMessagingTransport : IMessagingTransport
    {
        ...
        public static readonly int DefaultTimeout = 500;
            ...
            CancellationToken cancellationToken = default)
        {
            PushSubscribeOptionsBuilder psoBuilder = PushSubscribeOptions.Builder()
                 .WithStream(_natsOptions.Value.Stream);
            ...
            ccBuilder.WithFilterSubject("DEV.NBB.Contracts.PublishedLanguage.*");
        ...
        //    CancellationToken cancellationToken = default)
        //{
        //    PullSubscribeOptionsBuilder psoBuilder = PullSubscribeOptions.Builder()
        //         .WithStream(_natsOptions.Value.Stream);
        ...
        //    PullSubscribeOptions pso = psoBuilder.Build();
        ...
        //        //StreamMustExist(connection);
        //        IJetStream js = connection.CreateJetStreamContext();
        //        subscription = js.PullSubscribe(topic, pso);

        //        Task.Run(async () => await ReadMessagesAck(subscription as IJetStreamPullSubscription), cancellationToken);
        //    });

        //    return Task.FromResult(subscription);
        //}

        private async Task ReadMessagesAck(IJetStreamPullSubscription sub)
        {
            try
            {
                while (true)
                {
                    var message = sub.Fetch(1, DefaultTimeout).FirstOrDefault();
                    if (message != null)
                    {
